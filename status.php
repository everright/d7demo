...
$uid = db_query('SELECT uid FROM {users} WHERE uid = :uid', array(':uid' => 1), array('target' => 'default'))->fetchField();
if (1 != $uid) {
  $errors[] = 'Master database not responding.';
}
// Check that the slave database is active.
$uid = db_query('SELECT uid FROM {users} WHERE uid = :uid', array(':uid' => 1), array('target' => 'slave'))->fetchField();
if (1 != $uid) {
...
if ($test = tempnam($files_dir, 'status_check_')) {
